    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Your personalized timetable based on your subject selections
          </CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? (
                  ...
                  <tbody>
                    {DAYS.map((day, dayIndex) => (
                      <tr key={day} className="border-b last:border-0">
                        ...
                        {PERIODS.map((period) => {
                          const entry = getTimetableCell(dayIndex, period);
                          return (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-muted-foreground max-w-sm mx-auto">
                Your timetable hasn't been uploaded yet. Please contact your department administrator.
              </p>
            </div>
          )}
        </CardContent>
        ...
        <Card>
          ...
          <CardContent>
            <div className="flex flex-wrap gap-3">
              {Array.from(new Set(timetable.map((e) => e.subjectCode))).map((code) => {
                const subject = timetable.find((e) => e.subjectCode === code);
                return (
                  <div
                    key={code}
                    className={`px-3 py-2 rounded-md border ${getSubjectColor(code)}`}
                    data-testid={`legend-${code}`}
                  >
